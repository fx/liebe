...
describe('the icon-only tile’s accessible label', () => {
  /**
   * The one thing `iconOnly` hides rather than omits.
   *
   * Everything else the option suppresses is genuinely absent from the DOM, so
   * a rendered test can see it (`cardIconOnly.test.tsx`). The label is the
   * exception the contract requires — "the interactive surface stays fully
   * identified to assistive technology while the glyph alone identifies it
   * visually" (docs/specs/entity-cards/options/common.md — "Icon-only
   * presentation") — and *how* it is hidden is a property of these
   * declarations, invisible to jsdom, which is why it is asserted here.
   */
  it('takes the meta out of sight without taking it out of the accessibility tree', () => {
    const label = ruleBody(body, '.liebe-card-body-label')

    // Clipped to a 1px box rather than removed: the node still has a layout
    // box, which is what keeps it in the accessibility tree.
    ...
    // And out of the body's flow, so the glyph centres exactly as it would
    // with nothing beside it.
    expect(label).toContain('position: absolute;')

    /*
     * The three declarations that would each undo the whole point. Every one
     * of them hides the text visually AND removes it from the accessibility
     * tree, which is the one outcome this rule exists to avoid — and each is
     * the obvious "simplification" of the rule above.
     */
