...
describe('useServiceCall.setValue — input_datetime', () => {
  ...
  it('dispatches nothing when the value cannot serve the helper shape', async () => {
    // A time for a date-only helper: `set_datetime` would reject the payload, so
    // the branch reports rather than sending a guess.
    seedHelper('2024-01-15', { has_date: true, has_time: false })
    const { result } = renderHook(() => useServiceCall(), { wrapper })
    ...
    expect(callResult).toEqual({
      success: false,
      error: `Invalid input_datetime value for ${ENTITY_ID}`,
    })
    expect(result.current.error).toBe(`Invalid input_datetime value for ${ENTITY_ID}`)
