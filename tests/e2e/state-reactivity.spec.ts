...
import { openPanel, seedConfig, flagCardActive, setFlag, E2E_FLAG } from './helpers'
...
test('card reacts to an external state change without reload', async ({ page }) => {
  ...
  // Start from a known off state and confirm the card reflects it.
  ...
  await expect.poll(() => flagCardActive(page), { timeout: 15_000 }).toBe(true)
