...
- Configuration menu: theme picker (live apply, no reload), appearance control, custom-CSS textarea with the external-`url()`/`@import` warning per spec; invalid CSS falls back to browser error recovery without breaking the dashboard.
