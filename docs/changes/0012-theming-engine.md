...
- **Portalled overlays stay inside the theme scope** per the [spec's portal rule](../specs/theming/index.md#application-mechanism): existing Radix dialogs/dropdowns and the 0014 detail dialog portal to `document.body`, outside the shadow-root layers — this change MUST either provide a shadow-root portal host they mount into, or mirror all three synchronized layers into a scoped `.liebe-portal-root` document-level container (kept in sync on theme/appearance change), with a test asserting an open dialog renders active tokens.
  - **Delivered:** the base and theme layers reach document-level portals — the bundled baseline is linked into the owning document as well as the shadow root, and the theme layer is re-mirrored on every theme/appearance switch — so an open overlay renders on the active palette, asserted by e2e on a real overlay.
  - **Still owed:** neither branch of the requirement above is finished. There is no shadow-root portal host, no scoped `.liebe-portal-root` container, and the third layer — user CSS — is deliberately not mirrored, so a token override authored in custom CSS does not reach portalled overlays. The reasoning is [Portal mirroring stops at the theme layer](#design-decisions) below; the outstanding half is recorded as **"Not yet fully met — partly done"** against the [spec's portal rule](../specs/theming/index.md#application-mechanism), which is where the next change picks it up.
...
- **Portal mirroring stops at the theme layer** — the portal requirement above asks for a shadow-root portal host _or_ all three layers mirrored into a scoped document-level container. This change ships the first two layers into the owning document (the bundled baseline is linked there as well as into the shadow root; the theme layer is mirrored on every switch) and deliberately withholds the third: user CSS selectors are the author's own, nothing scopes them to Liebe, and a copy in the Home Assistant document would let an imported `body { display: none }` restyle the frontend around the panel. So overlays render on the active palette but not with custom CSS, and the scoped `.liebe-portal-root` host remains outstanding — recorded as "Partly done" against the [spec's portal rule](../specs/theming/index.md#application-mechanism) rather than silently satisfied. The e2e suite asserts both halves on a real open overlay.
