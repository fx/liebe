...
- **Dark appearance, for PRs 1–4.** The four defects this change was proposed for pass in dark; nothing in those tasks changes it, and a PR among them that moves a dark-appearance figure has overreached. **PR 5 is the exception, and it is why this bullet is now qualified:** the claim was made about the light-appearance repin and never measured per hue in dark, and `media` does not in fact clear the glyph floor there. Moving that one figure is PR 5's whole subject.
