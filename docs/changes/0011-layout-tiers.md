...
- Every card implements `glance` and `row`; cards whose option docs specify `tall`/`full` implement those too, as content placement only — controls that do not exist yet render nothing. **Exception: `CameraCard`**, whose sub-2×2 degradation is behavioral rather than layout (stream unmount, still thumbnail, lazy-mount fullscreen) and is owned by [0021](./0021-camera-presentation-options.md); here it keeps its current rendering at every span and receives its `data-tier` stamp without consuming it.
...
- `data-tier` is stamped on `liebe-card` for themes and tests.
