...
- [ ] **PR 2 — Detail dialog**: entity detail dialog (portalled), `more-info` wiring, hold default across cards; **password-helper redaction in the state display and attribute list, with a regression test** (the dialog must not ship able to reveal a secret the card masks); component tests + story; e2e hold flow
