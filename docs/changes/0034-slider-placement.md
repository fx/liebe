...
**Sequencing with [0042](./0042-tall-tile-control-geometry.md).** The forced placements land the omission path only on the axis the shell already publishes — the content width — which covers a forced `horizontal` slider at `tall`, the case with a real symptom. Two capacities stay unbounded here, and they are the same gap: the **long axis of a forced `vertical` slider**, whose band height nothing publishes, and a **row line's leftover width** once the icon circle and the gaps are out of the content region. A card may not measure the DOM for either, and establishing those signals is 0042 PR 3's, which owns the identical question for the placements the tiers choose on their own. Until it lands, a forced control is made cross-axis flexible so it narrows and shortens with the room it is given rather than overflowing it — the failure mode is a small control, never a clipped one, which is where the tiers' own controls already stand.
