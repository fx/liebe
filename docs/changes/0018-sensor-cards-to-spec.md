...
- **History aggregation mode is selected per consuming surface, not per entity**: `useEntityHistory` ([0015](./0015-history-and-forecast-data.md)) is requested with `mode: 'sample'` for line graphs — including on `total`/`total_increasing` sensors, whose line graph shows the cumulative series per the option doc — and with `mode: 'delta'` for the two surfaces that render differences: bar graphs and the trend arrow. Selecting the mode from `state_class` alone would make a default `graphMode: line` counter render per-bucket increments instead of its cumulative curve. A card showing both a bar graph and a trend arrow may share one delta projection, but a card showing a line graph and a trend arrow needs both projections of the same window — which is exactly why 0015 keys projection caching on mode.
