...
- The story matrix MUST cover each of the four domains × their states — idle, in-flight spinner, success check, error, `unavailable`, and the script running/tap-to-stop state — plus both values of `confirm` and `showLastActivated`, per [storybook — story coverage](../specs/storybook/index.md#story-coverage).
