...
- **The action map was established from Home Assistant, not from this document** — every service was read off a running instance's own registry (`GET /api/services` on 2026.7.2) and exercised against it before the map was written, rather than transcribed from the table above. That is why the defect turned out to be sharper than this document first described it, and why the count is three of four rather than four: `script.toggle` really does exist. `script.turn_off`'s unconditional registration — the fact behind "stoppable in all script modes" — was confirmed the same way, in the component source and its `services.yaml`. Recorded because the method is the transferable part: a card that calls a service the backend does not have fails identically to one that is never tapped, so a specification is not evidence that a service exists.
