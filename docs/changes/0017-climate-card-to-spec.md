...
- **PR 2's `displayUnit` needed the native unit to move.** Home Assistant core normalises climate values to `hass.config.unit_system.temperature` and does not put `temperature_unit` in a climate entity's state attributes — it is an entity property the Python API uses to convert the values it does publish, which is why it reads as an attribute in HA's entity documentation and is absent from what a card receives. The card read the attribute before this change, so the unit now comes from the unit system, with the attribute still honoured as a fallback for custom integrations that do publish one and Celsius as the last resort.
