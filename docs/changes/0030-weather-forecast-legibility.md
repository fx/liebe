...
- The icon-language rule is resolved per variant: the `default` variant's forecast glyphs move to the same family as its header icon (or its header moves to the glyph set) — one set per variant, decided in implementation, recorded in the option doc if the variant identity shifts.
