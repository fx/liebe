...
- **The touch floor and the portal host are scoped together deliberately.** Both are consequences of content escaping the shadow root and of Liebe's layered CSS meeting unlayered vendor CSS, and a portal host inside the shadow root may make part of the floor fix unnecessary. Sequencing: settle the layer question first (PR 1), because the portal host inherits whatever precedence model it establishes.
...
- **The label token MUST default to today's behaviour.** `--liebe-muted` inactive, `--liebe-fg` active, so Default and Liquid Glass are pixel-unchanged; only a theme that fills overrides it. The internal class names stay internal — this exposes the _decision_ they encode, not the selectors.
