...
**Spec:** [entity-cards](../specs/entity-cards/index.md) → [card dispatch and registry](../specs/entity-cards/index.md#card-dispatch-and-registry) and [card-reference](../specs/entity-cards/card-reference.md) · **Status:** complete · **Depends on:** —
...
- **Enforce the boundary rather than retire it.** The non-dashboard paths are real and growing — every card family now has a story matrix and a config preview, which are the two paths `GridView` does not cover — and a throw there currently takes down the workshop or the config modal rather than one card. Retiring is the cheaper decision and it trades a real safeguard for consistency with the majority that happens to be unprotected. The template already exists in four of them, so the work is mechanical.
  ...
  **Confirmed on the tree at implementation time, and both unprotected paths are concrete rather than hypothetical.** The registry maps 23 domains onto 20 distinct components — the action family's four domains share one card; four of the twenty wrap and sixteen do not, exactly as counted above. `CardConfig.tsx` renders `LightCard` and `BinarySensorCard` directly in its preview — two of the sixteen — and all 20 families have a story. The nearest boundary above a throwing card on the preview path is `Dashboard`'s, so a throw there blanks the entire dashboard rather than one tile; on the story path there is none at all.

- **One shared helper, not sixteen copies of the wrapper.** The established shape is a nine-line rationale comment plus a four-line wrapper function, and repeating it twenty times would be twenty copies of one contract — the duplication `REVIEW.md` calls this repo's largest defect class, already visibly drifting (`VacuumCard`'s copy cited a count of nine families and an issue this change supersedes). `withCardErrorBoundary` holds the rationale once and each card spends one call. The four that already wrapped are converted to it in the same PR: leaving two shapes for one convention is a smaller version of the state this change exists to end.

- **The unit is the registry entry, which includes the variants.** `getCardVariant` dispatches to a variant directly, so a variant is reached by exactly the paths a card is. `ClimateCard`'s `dial` had no boundary and is what the loader pins every climate card placed before change [0017](./0017-climate-card-to-spec.md) onto — the presentation most climate cards actually render. `WeatherCard` counted as wrapping because each of its four variants does, but those sit _below_ the dispatcher and cannot catch a throw in its option read, its store subscriptions or the `CardConfig.Modal` subtree it renders as the variant's sibling; it gets one of its own.

- **The test is generated from the registry rather than written per card.** Sixteen hand-written tests would satisfy the letter of the testing requirement and would still leave the seventeenth card to be noticed by review. `__tests__/cardErrorBoundary.test.tsx` enumerates `domainToCard` and each entry's `variants`, so a card or variant added without a boundary fails on the commit that adds it — the same reasoning as the reference document's path check in PR 2, applied to the convention instead of to the citations. It asserts the fallback rendered rather than merely that nothing threw: "did not throw" is also what a card that quietly rendered looks like.
