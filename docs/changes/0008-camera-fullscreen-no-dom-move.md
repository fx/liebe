...
- [x] Grid positioning regression verification — verify edit-mode drag, resize (all 8 handles), responsive breakpoint scaling, non-overlap, and visual appearance in the e2e environment and manually across cards (rides with the e2e task below) — covered by PR 1's merged browser verification and the e2e suite, which renders/interacts with the `absoluteStrategy` grid (`test/0008-e2e-no-reconnect-proof`)
...
- [x] E2E no-reconnect proof + spec updates — extended `tests/e2e/camera-stream.spec.ts` with the MutationObserver zero-detach assertion (armed on every root of the stream host's parent chain, shadow roots included), the no-new-negotiation assertions (no fresh HLS `master_playlist.m3u8` bootstrap via HTTP capture, no new `camera/webrtc/offer` via websocket-frame capture), the shadow-aware `elementFromPoint` topmost check at HA sidebar coordinates (guarded by a pre-fullscreen assertion the points are HA chrome), and strict playback-continuity checkpoints across all four toggle transitions, retaining viewport-coverage and `<home-assistant>`-ancestry assertions; updated [camera-streaming](../specs/camera-streaming/index.md) (Fullscreen section, Constraints — dropped the accepted-renegotiation concession and `KeepAlive` references, changelog entry) and [grid-layout](../specs/grid-layout/index.md) (positioning strategy) to match the implementation
