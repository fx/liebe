...
- The keys join the universal display option set end to end: display-key registry, per-key tolerant schema, defaults, the shared display config-form fragment, YAML round-trip — the same six surfaces every existing display option touches, so nothing special-cases these two.
