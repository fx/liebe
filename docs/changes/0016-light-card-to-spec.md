...
- [ ] **PR 2a — Light-color theming + dispatch migration**: bulb-colour resolution (`rgb_color`, then `hs_color` / `xy_color` / `color_temp_kelvin`, with the lightness clamp and the token fallback); the shell precedence composing it against an explicit `color` and the danger floor; `useLightColor` config-form entry; colour capability detection off `supported_color_modes`; **migration of the card's existing toggle and brightness dispatches onto the guarded path**, with the boundary-level single-call test including the early-acknowledgement case; unit tests; stories
