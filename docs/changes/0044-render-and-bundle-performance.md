...
**0001 narrowed the hot path and left the edges wide.** `useEntity`/`useEntities` subscribe to per-entity slices, but three whole-store subscriptions remain, each re-rendering its consumers on every batch:
...
- `useEntities` without a filter subscribes to the whole entity map (`useEntities.ts:48-50`), with a comment accepting it as the cost of needing the whole map. That was the right call when one screen needed everything; entity browsers and pickers now mount it alongside cards, so the "accepted cost" is paid by components that filter client-side.
...
- **Narrow without widening anywhere else.** Each narrowed subscription MUST select exactly the fields its consumers read today — `useConnectionStatus` splits into the existing slice hooks or a field-parameterized selector; the unfiltered `useEntities` path gets a key-scoped or memoized selection so a batch touching entities outside the consumer's interest does not re-render it; the dashboard default selector MUST NOT remain whole-state (narrow the default or remove it so callers name their slice). No consumer may observe a value it did not observe before.
...
- **No new spec sections.** Nothing here changes observable behavior — a dashboard that renders identically with fewer wakes needs no contract update. If a PR discovers a behavior the spec misstates (e.g. the unfiltered `useEntities` cost is load-bearing somewhere), that finding goes to the spec first.
...
- [ ] **PR 1 — Narrow the whole-store subscriptions**: slice `useConnectionStatus`, scope the unfiltered `useEntities` path, fix the dashboard default selector; render-count probes per path showing unrelated writes no longer wake consumers
...
- Does anything depend on the unfiltered `useEntities` whole-map identity (referential stability across batches) that a scoped selection would break? The probe suite should answer this; if yes, the memoization boundary needs stating.
