...
- **A configuration this build cannot fully interpret MUST survive a round-trip through it unchanged.** Within the fields of the portable contract, a value or key this build does not recognise **but accepts** MUST be carried through import → store → export exactly as it arrived; nothing may rewrite or drop it. (Which _top-level_ fields are serialized remains the canonical portable contract above — this rule governs what happens to their contents.) The observable test is round-trip identity: import a document carrying a key or a value no rule in this build mentions, export it again, and it is still there, in the field it arrived in, with the value it arrived with. Validation MUST therefore admit unrecognised keys instead of pruning to a known-keys set, and every load and migration step MUST carry the parsed document forward rather than reassembling one from the keys this build happens to know.
- **This rule starts where validation ends, and does not weaken it.** Where the schema deliberately rejects a value it does not know — an unrecognised grid item or widget `type`, a legacy `theme` scalar outside the three declared ones — that rejection is the contract, and the import fails loudly naming the field (see [Legacy Migration](#legacy-migration) for why an import and a corrupt localStorage are treated differently). The invariant governs the values validation _lets through_: those must not then be quietly repaired.
