...
**Status: implemented, except the layout tiers.** [0010](../../changes/0010-design-tokens-and-anatomy.md) delivered the whole of this spec bar one section: the [token contract](#token-contract) is declared for both appearances and exercised in the workshop (PR 1); the [card anatomy](#card-anatomy) parts — icon circle, meta block, pills, chips, big value, sparkline (PR 2) and the embedded slider in both orientations (PR 3) — carry their stable classes and attributes; and the card shell is a token-styled `liebe-card` tile (PR 4), which is also where the cards' inline visual declarations went away and the shell stopped being a Radix `Card variant="classic"`. Every existing card takes its **shell, icon circle, name and state line** from the anatomy, so those parts are reachable by the [stable selector contract](../theming/index.md#stable-selector-contract) on every card; each card's own interior — the thermostat's dial, the weather forecast rows, the camera's stream overlay — is still bespoke, and moving those onto the anatomy (or deciding they are content, which the domain-colour rule exempts) is the per-card wave that follows. The [size-adaptive layout tiers](#size-adaptive-layouts) remain unimplemented and are [0011](../../changes/0011-layout-tiers.md)'s: cards still take the legacy `size` prop, so `data-tier` is not yet stamped and the per-tier content rules below describe the target rather than what renders. The design was validated in a throwaway static mockup which this spec supersedes and replaces as the sole reference.
