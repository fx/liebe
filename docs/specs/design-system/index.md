...
Every entity card composes from a fixed set of anatomy parts, each with a stable, documented class name (`liebe-` prefix) so themes and tests can target them (see [theming — stable selectors](../theming/#stable-selector-contract)):
