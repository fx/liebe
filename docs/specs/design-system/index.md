...
- Every card MUST implement `glance` and `row`; `tall` and `full` are per-card (see per-card option docs under [entity-cards/options](../entity-cards/options/common.md)). **One standing exception:** the camera card renders the same content at every span and consumes no tier — it is stamped like every other tile, but what a camera does when it no longer fits is behaviour rather than layout (the stream unmounts, a still stands in, fullscreen mounts lazily), and that rule is owned by [camera-streaming](../camera-streaming/) with the presentation options in change [0021](../../changes/0021-camera-presentation-options.md).
