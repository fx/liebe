...
**Status: partially implemented — token contract and anatomy parts.** The [token contract](#token-contract) ships as of [0010](../../changes/0010-design-tokens-and-anatomy.md) PR 1: the `--liebe-*` properties are declared for both appearances and exercised in the workshop. The [card anatomy](#card-anatomy) parts — icon circle, meta block, pills, chips, big value, sparkline — ship as of PR 2 with their stable classes and attributes, each exercised by its own stories; no card composes from them yet. The embedded slider ships as of PR 3, in both orientations, on the unstyled Radix primitive. The card shell restyle (which migrates the cards onto the anatomy) is the remaining task of [0010](../../changes/0010-design-tokens-and-anatomy.md); the [size-adaptive layout tiers](#size-adaptive-layouts) are [0011](../../changes/0011-layout-tiers.md). Until those land, cards keep their stock Radix Themes styling (see [entity-cards](../entity-cards/)) and the rest of this document describes the target visual system. It was validated in a throwaway static mockup which this spec supersedes and replaces as the sole reference.
