...
- Every slider MUST expose a meaningful accessible name **on the element carrying `role="slider"`** — not on a wrapper around it. A name on the wrapper looks like a label in the markup while leaving the control anonymous to assistive technology, which is how the cards' sliders failed `aria-input-field-name` ([#192](https://github.com/fx/liebe/issues/192)). A slider MUST NOT be constructible without one; a plausible stand-in name ("Slider", "Value") satisfies the audit and tells a screen-reader user nothing, so it does not satisfy this rule.
...
| 2026-07-26 | Embedded slider implemented on the unstyled Radix primitive (both orientations, readout in the track); rule added that the accessible name belongs on the thumb                                          | [0010](../../changes/0010-design-tokens-and-anatomy.md) |
