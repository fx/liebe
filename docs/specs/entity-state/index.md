...
- ~~**A reconnect merges its snapshot rather than replacing the map, so `isMissing` cannot see a deletion that happened offline.**~~ **Resolved by change [0037](../../changes/0037-card-state-and-capability-correctness.md) PR 8.** `loadInitialStates` now writes through `replaceEntities`, which **reconciles the map against the snapshot** rather than merging into it — so an id the snapshot does not carry is, in the ordinary case, an id Home Assistant does not have, which is the fact `isMissing` needs and the merge threw away. `isMissing` reads the reconciled map, never raw snapshot membership: the two differ exactly where an entity was created after the snapshot was assembled. The replace **reconciles rather than clearing**, in both directions: an entity the snapshot carries is written unless the map already holds a strictly newer reading of it, and an entity the snapshot does **not** carry is kept if its reading is newer than every reading the snapshot does — the test that separates "created after this snapshot was assembled" from "deleted while the socket was down", since a deleted entity last changed before the disconnection and cannot clear that bar. Both comparisons are on Home Assistant's own `last_updated`, never on the browser's clock, so no skew between the two decides the answer. That is what keeps the fix from trading an under-report for an over-report, which would have been the worse direction — a card claiming "Entity Not Found" for an entity that exists is visible and wrong, where the old behaviour was merely stale.
...
| 2026-07-30 | A reconnect now REPLACES the entity map with its snapshot rather than merging into it, so an entity deleted while the socket was down is finally reported missing — the map had been a permanent superset of the state machine and `isMissing` blind to that whole class. The replace reconciles rather than clearing: an entity is written unless the map already holds a strictly newer reading, compared on `last_updated`, so an update racing the snapshot survives it. That ordering is the substance of the fix — a clear-and-apply would trade an under-report for an over-report, and a card claiming "Entity Not Found" for an entity that exists is the worse failure | [0037](../../changes/0037-card-state-and-capability-correctness.md) |
