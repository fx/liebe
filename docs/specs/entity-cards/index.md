...
| 2026-07-29 | Media player controls completed (change 0023 PR 2): `showVolume` with its degradation ladder (slider \u2192 buttons for a step-only player \u2192 mute alone for a mute-only one) and an optimistic drag that reconciles on any movement of the entity off the baseline recorded at dispatch \u2014 not on a match with the value sent, so a receiver that caps a request answers it rather than leaving the card insisting; `showSourcePicker` gated on `SELECT_SOURCE` **and** a non-empty `source_list`; `showProgress` deriving position from `media_position` + elapsed since `media_position_updated_at`, extrapolating only while playing and only when the bar renders, and **never** without a parseable timestamp; seek as a real slider where `SEEK` is advertised and a presentational bar where it is not; `artworkMode: background` with a scrim at `full`, degrading to the thumbnail below it with the stored option untouched. Config-form entries gate volume and source on capability but deliberately not artwork or progress, which depend on the session rather than the device.                             | [options/media-player](./options/media-player.md)                                       |
