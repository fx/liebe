...
- Every reading MUST come from one derivation the four variants share (`WeatherCard/presentation.ts`): the temperature conversion, the `secondaryInfo` fallback chain, the condition glyph and the artwork lookup. Variants differ in density and style, never in what a value MEANS.
