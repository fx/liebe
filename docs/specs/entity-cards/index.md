...
| 2026-07-29 | LockCard implemented — the first half of the security family, and a bugfix like the action family before it: `lock` fell through to the fallback, which dispatches `<domain>.toggle`, and the lock platform registers only `lock`, `unlock` and `open`, so every tap on a placed lock errored. The card defaults to `more-info` rather than a toggle, carries explicit Lock/Unlock pills with `confirmUnlock` on by default, and models `jammed` as its own state — neither locked nor unlocked, both pills live, and non-negotiably alert through the danger floor. The confirmation gate is written inverted: routes are classified by effect after action resolution and everything confirms except what is proven not to actuate this lock, so `lock.open` is gated with `lock.unlock` and the generic `homeassistant.*` aliases are held rather than guessed. Home Assistant's own `lock` component settled the state set and the alias behaviour, read off a running 2026.7.2 instance                                                                                                                                       | [0024-security-cards](../../changes/0024-security-cards.md)                             |
