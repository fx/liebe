...
**Status: implemented** by change [0027](../../changes/0027-scene-cards.md) PR 1. The per-domain action table, the option keys and defaults, the tier layouts and the chip anatomy live in [options/scene.md](./options/scene.md), which owns them; this section states only what holds of the card whatever it is configured to.

- One `ActionCard` component MUST be registered under **all four** of `scene`, `script`, `button` and `input_button` — four `domainToCard` entries pointing at one component, as `switch` and the fallback already share `ButtonCard`. The domains diverge only in which service a tap calls and in the script-only running state, so they MUST NOT be split into separate cards.
- The card MUST dispatch each domain's **own** service — `scene.turn_on`, `script.turn_on` (`script.turn_off` while running), `button.press`, `input_button.press` — and MUST NOT dispatch `toggle` on any of them. A stored `tapAction: toggle`, and `toggle` reached from any other gesture, MUST resolve to the domain's action rather than to `homeassistant.toggle`.
- **`unknown` MUST NOT be inert for `scene`, `button` and `input_button`.** Their state _is_ the last-activation timestamp, so a never-activated entity reports `unknown` and only an activation can move it out; treating it as inert would make every freshly created scene and button permanently unusable. `script` reports `on`/`off`, so `unknown` there is genuinely indeterminate and IS inert. `unavailable` is inert for all four.
- Every service this family calls is non-idempotent, so all four MUST dispatch through the non-retrying guarded path — a retried press presses twice, and a retried `script.turn_on` runs the script again ([options/common.md — dispatch guarantees](./options/common.md#action-type)).
- Because these entities expose no state change to observe, the card MUST confirm the tap itself: the icon swaps to a spinner while the call is in flight and to a success check held ~1.5s on success, with the active tint during the hold; a failure takes the standard error state and shows no check. This is intrinsic, not an option. Under `prefers-reduced-motion: reduce` the spinner animation and the swap transition MUST be suppressed **by the stylesheet** rather than by component logic, while the check still appears and still holds — it is evidence, not decoration.
- While a `script` is running the card MUST show a stop glyph, the active tint, and a state line offering the stop; tap MUST mean `script.turn_off`. The running state MUST supersede a pending success check. `script.turn_off` is registered unconditionally by Home Assistant, so a running script is always stoppable.
- `confirm` MUST gate every route that reaches the entity's own activation or stop services, classified **by effect** — the resolved `toggle` literal, a `call-service` naming those services directly, and the generic `homeassistant.toggle`/`turn_on`/`turn_off` aliases — and MUST name the action rather than asking about on/off ("Activate Movie Night?", "Run Water Garden?", "Stop Water Garden?"). Because a card family's rule _replaces_ the shell's generic on/off gate rather than joining it, this family's rule MUST cover everything that gate covered.
- The family MUST declare `defaultDimensions` of 1×1 — the first to do so — MUST embed no discrete control at any tier (the whole tile is the touch target), and MUST NOT expose a configuration modal of its own; its options render in the shared form, which surfaces `icon` prominently because a distinct per-card icon is this family's normal configuration path rather than an override.
- Registering these four domains changes what `getCardForEntity` resolves for **already-placed** grid items, so existing dashboards upgrade on load with no migration. That is deliberate: the fallback dispatches `<domain>.toggle`, which three of the four domains do not have — Home Assistant answers HTTP 400 for `scene.toggle`, `button.toggle` and `input_button.toggle`, while `script.toggle` exists and merely bypasses the running/stop distinction. Replacing a control surface that fails outright is a bugfix, not a replacement requiring pinning ([options/common.md](./options/common.md#conventions-for-per-card-options), convention 7). The new keys are additive, so existing configurations keep validating.

#### Scenario: A never-activated scene still activates

- **GIVEN** a `scene.movie_night` whose state is `unknown` because it has never run
- **WHEN** the user taps the card
- **THEN** it calls `scene.turn_on` for that entity and plays the spinner→check feedback — the card is not inert (`src/components/ActionCard/__tests__/ActionCard.test.tsx`, and end-to-end against a real Home Assistant in `tests/e2e/action-card.spec.ts`).
