...
**Status: implemented** by change [0016](../../changes/0016-light-card-to-spec.md). The option keys, their defaults, the tier layouts and the colour precedence live in [options/light.md](./options/light.md), which owns them; this section states only what holds of the card whatever it is configured to.

- `LightCard` MUST toggle the light on card click via `light.turn_on` / `light.turn_off`, and MUST decline that toggle while a brightness drag is in flight — the tile's tap target contains the slider, so without it dimming a light would also switch it off.
- Brightness MUST be presented on a 0–100 scale, converted to/from Home Assistant's 0–255 `brightness` attribute; committing 0 MUST turn the light off, and no nonzero percentage may round down to a `brightness` of 0 (which would be an off command in disguise). Drag state MUST stay local until commit, so a drag across the track sends one command rather than eighty.
- Every capability MUST be read from `supported_color_modes` first, with the legacy `supported_features` bits (brightness `1`, colour temperature `2`, colour `16`) consulted **only** when no usable mode list is published: brightness from `brightness` / `white` / `color_temp` / `hs` / `xy` / `rgb` / `rgbw` / `rgbww`, colour temperature from `color_temp`, colour from `hs` / `xy` / `rgb` / `rgbw` / `rgbww`. A mode list that is present and says no MUST be believed — a modern integration carrying a stale feature flag must not be given a control its own modes deny.
- A mode this build does not recognise MUST answer "no" to every capability question and MUST NOT prevent the card rendering; the same holds for a `supported_color_modes` that is not a list at all, which MUST fall back to the legacy bits rather than failing. A card that meets a newer Home Assistant loses a control at worst, never the tile.
- The colour-temperature control MUST span the entity's own reported `min_color_temp_kelvin`–`max_color_temp_kelvin` and MUST be withheld entirely when the entity does not report a usable pair — both bounds finite and positive with `min < max`. A substituted default range would offer a warm end the bulb may not reach and a cool end it may exceed. Kelvin is the only interface: Home Assistant Core 2026.3 removed the mired attributes and the `color_temp`/`kelvin` arguments, so there is no fallback path. Selecting a value MUST call `light.turn_on` with `color_temp_kelvin`.
- The colour control MUST call `light.turn_on` with `rgb_color`, and MUST mark a swatch as current only when the entity reports that exact `rgb_color`. A colour derived from `hs_color`, `xy_color` or a temperature is enough to tint the card but not to claim a swatch is what the light is set to — its payload would not reproduce the current state.
- Brightness presets MUST call `light.turn_on` with the converted `brightness` **even while the light is off**, since a preset means "turn on at N%"; and a preset MUST read as current only when the light is on and the level matches, because Home Assistant keeps the last `brightness` on an entity after it is switched off.
- Preset values MUST be resolved at render time, not repaired: values outside 1–100, fractional values and non-numeric entries are ignored, an empty result hides the row, and the stored document keeps every value its author wrote. `0` is not a valid preset — turning off is the tap action's job.
- Every dispatch — the toggle, the brightness commit, the temperature commit, a swatch and a preset alike — MUST go through the guarded, non-retrying path (common contract — dispatch guarantees). A repeated `turn_on` carrying a colour or a level is a command the user issued once.
- The bulb's own colour, when `useLightColor` allows it, MUST be _offered_ to the card shell rather than applied by the card: the shell composes it against a danger state and an explicit universal `color`, and every part that carries the tint MUST read the value that survived that precedence, so the icon and the slider cannot disagree about whether it applies.
- `LightCard` MUST expose a per-card configuration modal via its configure affordance.

#### Scenario: A preset turns an off light on at its level

- **GIVEN** an `off` dimmable light on a `full`-tier card with presets `[20, 50, 100]`
- **WHEN** the user taps `50%`
- **THEN** the card calls `light.turn_on` with `brightness ≈ 128` (`round(0.5 × 255)`) and does not merely toggle the light to its previous level.

#### Scenario: A colour-temperature bulb reporting no range gets no control

- **GIVEN** a light whose `supported_color_modes` includes `color_temp` but which publishes no `min_color_temp_kelvin` / `max_color_temp_kelvin`
- **WHEN** the card renders at `full` with `showColorTempControl: true`
- **THEN** no colour-temperature control appears, because the range it would span is unknown and MUST NOT be invented.
...
| 2026-07-29 | Light card completed — `useLightColor` bulb tinting composed against the danger floor and an explicit `color`, the warm→cool colour-temperature slider over the entity's **reported** Kelvin range (withheld entirely when it publishes none), the curated swatch row with its session-only recent slot, `brightnessPresets` acting on an off light, and every light dispatch on the guarded path                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | [options/light](./options/light.md), [entity-cards](./index.md)                         |
