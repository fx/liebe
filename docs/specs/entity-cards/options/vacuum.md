...
- **The chain is `batteryEntity`, then the derived sibling, then the legacy attribute.** The configured sensor comes **first**: `findBatterySibling` returns _a_ battery when a device exposes several, and correcting that pick is what `batteryEntity` is for, so an override that lost to the derivation could never do its job.
