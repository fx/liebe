...
- If the script is stoppable, tap MUST mean **stop** (`script.turn_off`) and the card MUST communicate that: the state line (or, in `glance`, the name line's place) reads "Running · tap to stop", and the icon SHOULD swap to a stop glyph for the duration. Scripts are stoppable via `script.turn_off` in all script modes; if a future mode disallows stopping, the card MUST fall back to an inert tap while running. **That fallback is deliberately not coded.** Home Assistant registers `script.turn_off` unconditionally — no `supported_features` gate and no mode check, in the component source as well as its `services.yaml` — so there is nothing an implementation could detect, and a branch keyed on a condition that cannot arise is untestable by construction. The clause stands as the rule to follow if such a mode ever appears, not as a gap.
...
- **Domain color for script/button.** Shipped as the fallback rule describes: the card maps `scene` to the `media` triplet and lets the other three resolve to `default`. Reason it went that way rather than sharing scene's indigo across the family: the [design-system table](../../design-system/index.md#domain-color-discipline) assigns indigo to scenes specifically and states a fallback for unlisted domains, and a card document cannot quietly extend another domain's row — the table is the single owner of that mapping. Still open only in the narrow sense that the family may eventually warrant a token of its own; that is a design-system change, and until the table carries one the fallback is the contract.
- **What "success" means for the check.** Answered as specified: the check confirms **dispatch**, not outcome — it is shown when the service call resolves, and Home Assistant acknowledges before slow integrations move. Reason it is not a defect: these entities publish nothing to observe, which is why the card owes the feedback at all, so there is no transition a generic card could wait for. Note this is a different question from the dispatch guard, which does hold until the entity moves (or the acknowledgement timeout elapses) — the guard and the check are independent, and only the guard's correctness depends on observing the entity. Whether a card could ever confirm outcome generically remains open; nothing in the four domains suggests it can.
