...
**Status: implemented** by change [0019](../../../changes/0019-cover-fan-cards-to-spec.md) PR 1, on the tier layouts change [0011](../../../changes/0011-layout-tiers.md) PR 3 laid down. Every option key below is live, edited from the shared configuration form and capability-gated from the entity: the position options are offered only to a cover advertising set-position, the tilt option only to one advertising a tilt bit, and `confirmOpen` only to the three security device classes. `stateLabelStyle` stores the two concrete styles and derives the third from the entity by leaving the key absent. Implementing it also corrected two feature bits the card had transposed — stop-tilt is `64` and set-tilt-position is `128` — so a stop-tilt-only cover no longer renders a tilt slider it cannot drive. See [entity-cards — Covers and fans](../index.md#covers-and-fans) for the card-level requirements. One row below is the exception to this status: `sliderPlacement` is _specified by change [0034](../../../changes/0034-slider-placement.md), not yet implemented_ — the schema and configuration form do not carry it yet.
...
| -------------------- | ---------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `showPositionSlider` | boolean                            | `true`                                                          | Renders the position slider when the entity supports set-position (bit 4). Tiers: `row` (horizontal), `tall` (vertical), `full` (horizontal); never in `glance` for inline placements — `sliderPlacement: background` renders in every tier, `glance` included ([shared contract](./common.md#shared-slider-placement-sliderplacement)).                                                                                                             |
| `sliderPlacement`    | select                             | `auto`                                                          | Placement/orientation of the position slider, per the [shared slider-placement contract](./common.md#shared-slider-placement-sliderplacement). Vertical and `background`-vertical fills keep **top = open** (the slider mirrors the physical opening); `background` drags on a security opening pass through the `confirmOpen` gate like any other position commit. _Change [0034](../../../changes/0034-slider-placement.md), not yet implemented._ |
...
| `stateLabelStyle`    | select: `percent` \| `open-closed` | `percent` for positional covers, `open-closed` for binary ones  | Position display style on the state line. All tiers.                                                                                                                                                                                                                                                                                                                                                                                                 |
...
### State label style (`stateLabelStyle`)
...
- **This key was `stateLabels` until change [0038](../../../changes/0038-option-key-collision.md).** It was renamed because the switch and fallback cards document [`stateLabels`](./switch.md#statelabels) as their own `{ onLabel, offLabel }` text pair, and one shared item schema cannot govern both shapes. Cover cards stored before the rename keep working: the loader renames a **string** value on a **cover** card only — an object under that key is the switch family's option and is left alone — and never writes the legacy key back, per [dashboard-config — file import & version handling](../../dashboard-config/index.md#file-import--version-handling).
...
| Tier     | Content                                                                                                                                                           |
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `glance` | Icon circle + name + state line (position % per `stateLabelStyle`); whole tile is the tap action. No embedded controls.                                           |
...
- `src/components/CoverCard/` — the card (`index.tsx`), what its options resolve to (`presentation.ts`), and its detail-dialog controls (`CoverDetailControls.tsx`); the stored option contract is `src/store/coverOptions.ts`
