...
**Status: implemented** by change [0019](../../../changes/0019-cover-fan-cards-to-spec.md) PR 1, on the tier layouts change [0011](../../../changes/0011-layout-tiers.md) PR 3 laid down. Every option key below is live, edited from the shared configuration form and capability-gated from the entity: the position options are offered only to a cover advertising set-position, the tilt option only to one advertising a tilt bit, and `confirmOpen` only to the three security device classes. `stateLabelStyle` stores the two concrete styles and derives the third from the entity by leaving the key absent. Implementing it also corrected two feature bits the card had transposed — stop-tilt is `64` and set-tilt-position is `128` — so a stop-tilt-only cover no longer renders a tilt slider it cannot drive. See [entity-cards — Covers and fans](../index.md#covers-and-fans) for the card-level requirements. One row below is only partly covered by this status: `sliderPlacement` arrived with change [0034](../../../changes/0034-slider-placement.md) PR 1, which carries the schema and the configuration row for `auto`, `horizontal` and `vertical`; its `background` value is implemented by change [0034](../../../changes/0034-slider-placement.md)'s second task.
...
- **This key was `stateLabels` until change [0038](../../../changes/0038-option-key-collision.md).** It was renamed because the switch and fallback cards document [`stateLabels`](./switch.md#statelabels) as their own `{ onLabel, offLabel }` text pair, and one shared item schema cannot govern both shapes. Cover cards stored under the legacy key keep working, per [dashboard-config — file import & version handling](../../dashboard-config/index.md#file-import--version-handling), which owns what happens to a renamed key on the way in.
