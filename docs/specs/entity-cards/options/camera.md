...
- **Where each line lives:** the overlay's name line REPLACES the status pill's own name rather than joining it — a camera named twice over its own picture is the duplication the overlay exists to remove — and `hideName` removes it from both, never handing it back to the pill. The pill's status line is the STREAM's health (`CONNECTING`, `NO SIGNAL`), owned by [camera-streaming](../../camera-streaming/), and is NOT what `hideState` hides; `hideState` hides the overlay's entity-state line. Where the overlay is not drawn at all — a camera with no stream support, or the error-and-Retry branch — the name returns to the pill, which is then the only place for it.
- **Badge subsumption in the pill:** while the badge renders, the pill drops its own live label (and its pulsing dot) so live-ness is claimed exactly once; `streaming` presents as `LIVE` and `recording` as `REC`, keeping the distinction the pill drew. Every other status stays on the pill unchanged. The badge is gated on a MOUNTED stream element, not on the status alone — the status machine resolves `recording` from the raw entity state, which the still-image fallback also reaches.
