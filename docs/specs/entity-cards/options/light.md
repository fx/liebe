...
| ---------------------- | -------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `showBrightnessSlider` | boolean  | `true`  | Renders the brightness slider when the entity supports brightness. Tiers: `row` (horizontal), `tall` (vertical), `full` (horizontal); never in `glance` for inline placements — `sliderPlacement: background` renders in every tier, `glance` included ([shared contract](./common.md#shared-slider-placement-sliderplacement)).                                                                         |
| `sliderPlacement`      | select   | `auto`  | Placement/orientation of the brightness slider, per the [shared slider-placement contract](./common.md#shared-slider-placement-sliderplacement). Under `background`, the fill follows the same colour resolution as the slider today — bulb colour under `useLightColor`, domain token otherwise. `background` is _change [0034](../../../changes/0034-slider-placement.md)'s second task, implemented_. |
...
- When `true` and the light is `on` with a resolvable RGB color (`rgb_color`, or derivable from `hs_color` / `xy_color` / color temperature), the icon-circle tint and the slider fill MUST use that color instead of the domain token. It reaches the tint and the fill and stops there — the glyph inside the circle is a foreground on a 20% veil of that same colour, and takes the neutral one per [design-system — domain color discipline](../../design-system/index.md#domain-color-discipline). The list in the sentence above was always exhaustive; what change [0035](../../../changes/0035-light-appearance-contrast.md) PR 4 fixed was an implementation that had read it as illustrative, and rendered a bulb reporting white as a white glyph on a white wash at 1.01:1.
