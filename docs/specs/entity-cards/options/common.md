...
| `color`           | select  | `auto`      | Canonical enum, persisted verbatim: `auto \| light \| heat \| cool \| ok \| alert \| media \| vacuum \| water \| default` — each non-`auto` value maps to the token `--liebe-c-<value>` ([design-system color table](../../design-system/#domain-color-discipline)). `auto` uses the card's state-aware domain resolution; a named value pins that single token for the card's active treatment. No other values are valid (schema-validated). |
...
- `hideName` and `hideState` MUST compose with the layout tiers ([design-system — size-adaptive layouts](../../design-system/#size-adaptive-layouts)): hiding both in `glance` tier leaves an icon-only tile, which MUST remain a valid layout with a centered icon.
