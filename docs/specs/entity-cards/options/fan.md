...
- `steps` renders discrete equal-width pills (`liebe-pill`) derived from the entity's `percentage_step`: the step values are the exact multiples of `percentage_step` from one step up to 100, with the top value clamped to the largest multiple ≤ 100 (e.g. `percentage_step: 25` → pills 25 / 50 / 75 / 100). Non-divisor steps are deterministic too: fractional multiples are computed internally but **both label and payload are the rounded integer**, and the top pill is pinned to `100` (`percentage_step: 33.33` → pills 33 / 67 / 100, sending exactly those values). `fan.set_percentage` coerces `percentage` to an integer in 0–100, so sending `99.99` would truncate to `99` — a pill labeled 100 that never reaches full speed and a selected state disagreeing with the device. The selected pill is the one nearest the current `percentage` within half a step. When `percentage_step` is absent, non-positive, or would yield more steps than fit the tier, the card MUST fall back to four quartile pills (25/50/75/100). The pill nearest the current percentage renders as selected (active tint pattern). Tapping a pill MUST call `fan.set_percentage` with that value.
...
- ~~**Non-divisor `percentage_step`.**~~ Resolved in the [speed control](#speed-control-speedcontrol) section and implemented by change [0019](../../../changes/0019-cover-fan-cards-to-spec.md) (explicitly not deferred): for steps like `33.33`, label and payload are both the rounded integer with the top pill pinned to 100 (33 / 67 / 100), since `fan.set_percentage` coerces to an integer and would truncate `99.99` to `99`; the selected pill is the multiple nearest the current `percentage` within half a step. Only the label ergonomics on real 3-speed hardware (whether users expect `33` or a `Low`/`Medium`/`High` naming) may warrant a follow-up option; the arithmetic contract is settled.
