...
- ~~**Non-divisor `percentage_step`.**~~ Resolved in the [speed control](#speed-control-speedcontrol) section and implemented by change [0019](../../../changes/0019-cover-fan-cards-to-spec.md) (explicitly not deferred): for steps like `33.33`, **labels** round to the nearest integer while **payloads** send the exact multiple (33 / 67 / 100 labelled, 33.33 / 66.66 / 99.99 sent), and the selected pill is the multiple nearest the current `percentage` within half a step. Only the label ergonomics on real 3-speed hardware (whether users expect `33` or a `Low`/`Medium`/`High` naming) may warrant a follow-up option; the arithmetic contract is settled.
