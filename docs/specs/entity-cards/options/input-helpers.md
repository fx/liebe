...
- **A retained control MUST be operable without a pointer.** Wherever a card keeps a control because removing it would leave no way to operate the entity, that control MUST be a real button (or a component that renders one): in the tab order, carrying an accessible name that says what it does, and activating on both Enter and Space. A readout that is a `div` with a click handler satisfies the retention for a mouse and fails it for every keyboard, switch and screen-reader user — which is the population the no-operability-regression invariant most exists for, since they have the fewest ways around a tile they cannot reach.
- **The control-free `glance` above is not yet what ships, for the four non-boolean helpers.** Every `glance` row that falls back to `more-info` assumes the detail dialog exposes the helper's full control — and those dialog controls are registered by change [0022](../../../changes/0022-switch-input-helpers-to-spec.md), not before. Until then a control-free 1×1 `input_number`, `input_select`, `input_text` or `input_datetime` would have **no way to operate the entity at all**, which the no-operability-regression invariant forbids at a merge point, so change [0011](../../../changes/0011-layout-tiers.md) keeps each card's existing minimal control in `glance` and omits secondary content instead. For `input_number` that means the click-to-edit readout stays and the `+`/`-` buttons go: the readout sets any value in the range where the buttons only step by one, and two ≥40px targets beside a readout and a name do not fit one cell. `input_boolean` is the exception that already matches this table — its `glance` control was removed on schedule, because the whole-tile toggle from [0014](../../../changes/0014-universal-card-options.md) carries its operability. **0022 completes the control-free `glance` for the other four in the same PR that registers their dialog controls**; until that PR lands, this table describes the target and not the build.
