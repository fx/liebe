...
Part of the [entity-cards spec](../index.md); builds on the [common contract](./common.md) (universal options are not repeated here). **Status: implemented.** Change [0022](../../../changes/0022-switch-input-helpers-to-spec.md) delivered the `controlStyle` options, the tier layouts below including the control-free `glance`, and the five helpers' controls in the detail dialog's domain control slot. The `tall` control rule under [`input_number`](#input_number) landed in change [0042](../../../changes/0042-tall-tile-control-geometry.md) PR 2: a card set to `stepper` renders the vertical slider at that tier rather than a stepper the tile clips. Change [0037](../../../changes/0037-card-state-and-capability-correctness.md) PR 5 closed the last gap the head called out, the `input_number` and `input_select` tap-to-focus primary actions.
...
- **`input_number`** — **focus the value control**: in tiers showing the stepper, focus the click-to-edit value field (entering edit state); in tiers showing the slider, focus the slider thumb. **Wherever no embedded control renders, `default` MUST fall back to `more-info`**, where the detail dialog exposes the full control — `glance`, which never has one, and equally any tier where the control was omitted for want of room ([design-system — cross-axis fit](../../design-system/index.md#cross-axis-fit)). The rule is stated on the absence of a control rather than on the tier because those are the same case: there is nothing to focus either way. The `more-info` fallback holds at `glance`; the focus behaviour holds at every other tier, consulting the resolved presentation rather than the stored `controlStyle` ([0037](../../../changes/0037-card-state-and-capability-correctness.md) PR 5).
- **`input_select`** — **open the control**: open the dropdown (or, in pill presentation, focus the pill group). In `glance`, fall back to `more-info` — and equally wherever the dropdown renders disabled with no options to offer, where focusing the trigger would neither open a control nor operate the helper. Built past `glance`, as above ([0037](../../../changes/0037-card-state-and-capability-correctness.md) PR 5).
- **`input_text`** — **focus the text field**, entering inline edit state. **Wherever no field renders, fall back to `more-info`** — `glance`, which never has one, and `tall`, where the tier omits it for want of room ([design-system — cross-axis fit](../../design-system/index.md#cross-axis-fit)). Stated on the absence of a field rather than on the tier, because those are the same case: there is nothing to focus either way, and a stored `tapAction: toggle` resolves there too.
- **`input_datetime`** — **open the native date/time picker** on the embedded input. **Wherever no input renders, fall back to `more-info`** — `glance` and `tall` alike, on the same reasoning as `input_text` above.
...
- **`stepper` MUST NOT render at `tall`; the vertical slider renders there instead.** The stepper is a content-sized row of buttons around a value field, and a `tall` tile is one column wide — so it cannot be narrowed to the tile's content region without dropping its buttons under the touch floor, and rendering it anyway clips it against the tile's own edge ([design-system — cross-axis fit](../../design-system/index.md#cross-axis-fit), where change [0042](../../../changes/0042-tall-tile-control-geometry.md) measured it). The fallback is presentation only: the option keeps its stored value, `row` and `full` are untouched, and the card returns to the stepper as soon as it is that wide again — the same shape as `input_select`'s pills falling back to the dropdown outside `full`. **What it costs is typed entry, and only for one configuration.** A `box`-mode helper keeps it: the [detail dialog](#primary-action) resolves its control from the helper's own `mode`, so a hold on the tile (`holdAction` defaults to `more-info`) reaches the stepper. A `slider`-mode helper whose card explicitly stores `controlStyle: 'stepper'` does not — the dialog is opened for an entity and never for a card, so it resolves presentation from `mode` and cannot see the override, however the card is configured. "The same control" above binds the implementation the dialog mounts, not which of the two presentations it picks. Nothing typed therefore remains at `tall` for that combination, and a card configured that way SHOULD be given two columns rather than one.
...
| `input_number`   | **Value big** — the current value as the large numeric readout (`liebe-value`, tabular-nums) with unit muted; no control; tap → more-info | Icon + meta + **stepper** (or horizontal slider per `controlStyle`) | Icon top, **always the vertical slider** regardless of the stored `controlStyle`, meta bottom | `row` control plus the `min – max` range line                            |
...
| `input_text`     | Icon + name + value as state (masked if password); tap → more-info                                                                        | Icon + meta + inline text field                                     | Icon top, meta bottom, **no field** — one column cannot hold one; tap → more-info             | As `row`, wider field                                                    |
| `input_datetime` | Icon + name + formatted value / `(not set)`; tap → more-info                                                                              | Icon + meta + native input(s)                                       | Icon top, meta bottom, **no input** — one column cannot hold one; tap → more-info             | As `row`; datetime helpers MAY split date and time inputs onto two lines |
...
- **`input_text` and `input_datetime` render no embedded input at `tall`, and their tap resolves to `more-info` there** — including a configured `tapAction: toggle`, which would otherwise find nothing to focus. Both inputs are bounded by their own content on the axis that tier is one cell wide, so they are omitted rather than narrowed; [design-system — cross-axis fit](../../design-system/index.md#cross-axis-fit) owns the rule and the measurements, and change [0042](../../../changes/0042-tall-tile-control-geometry.md) PR 4 the audit that applied it here.
