...
- **`input_number`** — **focus the value control**: in tiers showing the stepper, focus the click-to-edit value field (entering edit state); in tiers showing the slider, focus the slider thumb. **Wherever no embedded control renders, `default` MUST fall back to `more-info`**, where the detail dialog exposes the full control — `glance`, which never has one, and equally any tier where the control was omitted for want of room ([design-system — cross-axis fit](../../design-system/index.md#cross-axis-fit)). The rule is stated on the absence of a control rather than on the tier because those are the same case: there is nothing to focus either way. **Status: the `more-info` fallback is held at `glance`; the focus behaviour is not built at any other tier**, where the tap does nothing today ([0037](../../../changes/0037-card-state-and-capability-correctness.md)).
