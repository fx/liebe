...
Part of the [entity-cards spec](../index.md); builds on the [common contract](./common.md) (universal options are not repeated here). **Status: implemented, except the `input_number` and `input_select` tap-to-focus primary actions and `input_number`'s `tall` control.** Change [0022](../../../changes/0022-switch-input-helpers-to-spec.md) delivered the `controlStyle` options, the tier layouts below including the control-free `glance`, and the five helpers' controls in the detail dialog's domain control slot.

**Implemented is not the same as met.** Two rules this status calls shipped are not held, and the flip does not close either:

- **The `tall` control rule under [`input_number`](#input_number) is target behaviour, not a description of what renders.** Change [0042](../../../changes/0042-tall-tile-control-geometry.md) PR 1 settled it; its PR 2 is what implements it. Today a card set to `stepper` still renders the stepper at `tall`, where the tile clips it.
