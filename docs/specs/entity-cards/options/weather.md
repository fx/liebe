...
- Text and glyphs over the artwork MUST satisfy the [design-system content-imagery scrim rule](../../design-system/index.md#card-anatomy), which owns the floor and its rationale. Weather-specific application only: the scrim applies wherever the card paints condition artwork — every tier, every artwork-bearing variant — shadows MAY remain as accent on top, and the shipped shadow-only treatment is the open defect [#215](https://github.com/fx/liebe/issues/215). _Change [0030](../../../changes/0030-weather-forecast-legibility.md), not yet implemented._
