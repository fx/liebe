...
- When `true` and the condition→image map resolves an image for the entity's state, the card MUST render it as a cover background with white text/icons ([entity-cards — Weather](../index.md#weather)). The card's own backdrop blur is turned off while artwork renders, so the photograph is not read through frosted glass.
- Text and glyphs over the artwork MUST sit on a scrim per [design-system — content-imagery legibility](../../design-system/index.md#card-anatomy): the shipped shadow-only treatment measures 2.64:1 ([#215](https://github.com/fx/liebe/issues/215)) and does not satisfy the 4.5:1 floor. The scrim replaces shadows as the legibility mechanism (shadows MAY remain as accent); the media player's backdrop is the reference treatment. _Change [0030](../../../changes/0030-weather-forecast-legibility.md), not yet implemented._
...
- **The card MUST use one icon language throughout**: the forecast columns' condition glyphs and the card's own condition icon come from the same set, per variant. A variant whose header is an emoji MUST NOT pair it with line-art forecast glyphs (the shipped `default` variant does exactly this).
