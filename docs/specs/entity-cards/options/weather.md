...
- A section its tier has no room for, or whose option is `false`, MUST subscribe to nothing — a strip that cannot be drawn needs no fetch, no cache entry and no refresh timer behind it. Availability is resolved per type, on separate subscriptions, so an entity publishing hourly data and no daily view still shows its hourly strip.
