...
| `showConditionBackground` | boolean                                                            | `true`     | Renders the condition-mapped background image with the scrim treatment, whose foreground arrives through the theme's tokens ([Condition background](#condition-background-showconditionbackground)). All tiers except where the variant opts out (`minimal` today). |
