...
| ButtonCard          | `switch` + fallback   | 2×1 (`src/components/ButtonCard/index.tsx`)      | No                         | toggle                        | `src/components/__tests__/ButtonCard.test.tsx`                                        |
...
Implemented to the [light option contract](./options/light.md) by change [0016](../../changes/0016-light-card-to-spec.md); that document owns the keys, defaults and tier placements. The card lives in `src/components/LightCard/`, with the pure parts split out: `lightColor.ts` (bulb colour resolution and the Kelvin→RGB fit), `lightCapabilities.ts` (the capability matrix and the reported Kelvin range), `lightPalette.ts` (the swatches and the reported-colour read).
