...
- Shadow-root styling depends on locating the running script via `document.currentScript || document.querySelector('script[src*="panel.js"]')` and resolving its containing directory with `new URL('./', script.src)` (`src/panel.ts:151`), so a cache-busting query or hash on `module_url` does not leak into derived asset URLs. The fallback selector still matches on the literal substring `panel.js`, so if the bundle is not served from a URL containing `panel.js`, CSS and `__LIEBE_ASSET_BASE_URL__` resolution break.
...
- **Coverage stops at first mount.** `src/__tests__/panel.test.ts` exercises only initial mount: that the element is registered under `getPanelConfig().elementName`, that it attaches a shadow root holding the `data-liebe-root` React container, and that it publishes `__LIEBE_ASSET_BASE_URL__` plus the `liebe.css` `<link>` resolved from the directory `panel.js` was served from (including when `module_url` carries a cache-busting query). Everything after that is unverified: the overridden `remove()`, the parent observer, the three intervals, the global guardian, the `hass` setter, and the visibility handler have no tests, and neither do `useIsHomeAssistant` or the path helpers in `config/panel.ts` (`getAllPanelPaths`, `isPanelPath`, `getPanelBasePath`). `HomeAssistantContext` is only pulled in as a wrapper or type by other suites, never asserted on directly. Regressions in the resurrection/keep-alive path are still caught only manually in a live HA instance.
