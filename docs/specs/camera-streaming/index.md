...
| `CameraOverlay.tsx`        | Name/state gradient band and the `LIVE` badge — siblings of the stream node, never wrappers                |
| `CameraThumbnailTile.tsx`  | The degraded tiers' still thumbnail + name/state, laid out by the shared `CardBody`                        |
| `overlay.ts` / `tiers.ts`  | Pure presentation and tier rules — overlay composition, badge honesty, motion wording, the 2×2 boundary    |
| `CameraCard.css`           | Recording-dot pulse, the presentation layers, and the thumbnail's tile-relative sizing                     |
...
- **Degraded tiers below 2×2 — no stream, lazy fullscreen (change [0021](../../changes/0021-camera-presentation-options.md)).** The no-DOM-move invariant above governs a stream that _is_ mounted. Below a 2×2 effective span the card mounts no stream element at all and shows the still `entity_picture` thumbnail, so there is nothing to preserve: entering fullscreen from a degraded tier MUST mount the stream element fresh **if the stream path is otherwise open** — the tier is one more term in `streamEnabled`, not a replacement for it, so the camera must still support streaming, be connected, and have the bootstrap ladder report `ready` (see "Stream Support Detection"). Exiting MUST unmount it and return to the thumbnail. A new connection on that path is correct rather than a regression — no connection existed. This is the sole exception to the persistently-mounted rule, it applies only below 2×2, and the ≥2×2 no-reconnect guarantee is unaffected. Presentation of the degraded tiers is owned by [entity-cards options — camera](../entity-cards/options/camera.md).
  - **The status machine is disabled with the element**, not left running against nothing: the tile's `enabled` input is false, so no load budget is armed, no watchdog runs, and no status is surfaced for a stream that does not exist. Entering fullscreen enables it again from a clean state; exiting disables it, and the existing disable-reset rule wipes any status the short-lived stream surfaced. A degraded tile therefore costs one periodically refreshed snapshot and nothing else.
  - **A camera the stream path is closed for behaves in fullscreen exactly as it does anywhere else**, because nothing about that path is tier-specific: a camera without `SUPPORT_STREAM` shows its static video icon at every span and in fullscreen, and one whose element cannot be bootstrapped (readiness `unavailable`) shows the still-image fallback — which in fullscreen is the same snapshot the tile was already showing, promoted to fill the viewport. Neither is a degraded-tier rule; both are the ordinary rules, unchanged.
  - **One tile decides, not one card:** the boundary is the tier derived from the item's EFFECTIVE span (`docs/specs/design-system` — "Size-adaptive layouts"), so a card that a breakpoint change collapses to one cell stops streaming, and one that grows past 2×2 starts. The card never measures itself to find out.

#### Scenario: A small tile mounts no stream, and fullscreen mounts one

- **GIVEN** a stream-capable camera card, its element bootstrapped, resized to a 1×1 effective span
- **WHEN** the tile renders, the user taps it, and then leaves fullscreen
- **THEN** no `<ha-camera-stream>` exists while the tile is shown (the thumbnail stands in and the status machine is disabled), one is mounted on entering fullscreen and plays, and it is unmounted again on exit — the ≥2×2 no-reconnect guarantee being untouched, since this path never had a connection to keep
...
- **go2rtc sidecar** (`alexxit/go2rtc:1.9.14`, `ha/docker-compose.yml`): publishes `e2e_pattern`, a fully synthetic ffmpeg `testsrc2` 1280×720@15 stream defined in `ha/go2rtc/go2rtc.yaml` — deterministic, no network access. Only the WebRTC media port is published to the host — on a per-checkout port derived alongside Home Assistant's ([../architecture/](../architecture/#end-to-end-harness), change 0040), advertised to peers as the candidate port so the two cannot disagree; the API (1984) and RTSP (8554) ports stay internal, reached by HA via the `go2rtc` hostname.
...
| 2026-07-27 | Degraded tiers below 2×2 documented and implemented: the tile mounts no stream element and disables the status machine with it, fullscreen mounts one lazily on entry and unmounts it on exit. The sole exception to the persistently-mounted rule; the ≥2×2 no-reconnect guarantee is unchanged, and the e2e now proves both — that resizing past the boundary removes the element, and that presentation-option toggles never disturb it                                                                                                                                                                                                                                                                                                                                    | [0021](../../changes/0021-camera-presentation-options.md)   |
