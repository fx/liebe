...
- Custom CSS MUST be applied as-authored into the user layer, with one exception: **`@import` rules MUST be rejected** — stripped at injection time with an explicit editor validation error naming each stripped import (never silently). Top-level `@import` cannot legally appear inside the `@layer liebe-user { … }` wrapper, and imports fetch external resources, which violates the offline constraint. External-origin references MUST likewise be stripped at injection with an editor notice — and the boundary is enforced by **resolution, not pattern-matching on tokens or schemes**: browsers fetch through multiple constructs (`url()`, `image-set()`, `src()`, and future forms) and accept scheme-less forms (protocol-relative `//host/…`) and CSS-escaped text. The sanitizer MUST therefore CSS-unescape each candidate resource reference, resolve it against the document base URL, and drop any declaration whose **resolved** reference is not same-origin or `data:` — same-origin-resolving relative paths and `data:` URIs pass through, everything else (scheme-full, protocol-relative, escaped variants) is stripped. The scan applies to **every declaration, including custom-property definitions**: a url-shaped value (quoted string or token, escaped or not) inside `--any-property: …` is a candidate reference and is dropped at the definition site — otherwise `--remote: "https://…"` later consumed by `image-set(var(--remote) 1x)` would smuggle an external fetch past a consumption-site-only scan. The indirection case MUST have a sanitizer test. **Inherited-variable indirection is likewise closed, and the check MUST be recursive.** Such variables inherit through the shadow host, so an outer Home Assistant theme could resolve them to an off-origin `url(…)` the static scan never saw. Define a user-defined custom property as **clean** only when all three hold: (1) its own value survives the url-resolution scan above, (2) every `var()` in its value references either a `--liebe-*` token or another **clean** user-defined property, and (3) its value is not a CSS-wide keyword that resolves to an outer value at computed-value time — `inherit`, `unset`, `revert`, and `revert-layer` are **unclean by definition**. A definition like `--lovelace-background: inherit` contains no URL and no `var()`, so a syntactic scan reads it as clean, yet it launders whatever the outer Home Assistant theme holds — including an off-origin `url(…)` — into any Liebe token that then references it. Properties that are undefined in the user CSS, inherited from outside it, or part of a reference cycle are **unclean**, and uncleanliness propagates — cleanliness is the transitive closure, not a one-hop lookup. A fetch-capable declaration (background/mask/border-image/content/cursor and kin) whose value references anything unclean MUST be stripped with an editor notice. **Unclean custom-property definitions MUST themselves be dropped at the definition site**, including overrides of `--liebe-*` tokens — stripping only at fetch-capable declarations is not enough, because base and theme CSS already consume those tokens in fetch-capable positions: `:host { --liebe-card-bg: var(--ha-image) }` contains no fetch-capable declaration of its own, yet the base card rule's `background: var(--liebe-card-bg)` resolves it off-origin. A definition is the reference; it does not need a user-authored consumer. A sanitizer test MUST cover a token override consumed from base/theme CSS. A one-hop check is insufficient and MUST NOT be implemented: `--local: var(--ha-url); background-image: var(--local)` passes it — the definition holds no url text to scan, and the consumption site references a property that is technically user-defined — while still resolving off-origin at render time. Sanitizer tests MUST cover the direct inherited reference, this delegated case, a reference cycle, and each CSS-wide keyword. Imported YAML applies custom CSS immediately, so a merely-advisory warning would let an untrusted shared config initiate remote requests (network metadata leak, visual spoofing) before the user ever sees the editor — and remote assets violate the offline constraint regardless. Everything else needs no sanitization (config is user-owned once reviewed).
