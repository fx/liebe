...
Three things the sections below specify are **not** delivered by that, and are not closed by this status:

- Custom CSS does not reach portalled overlays — the user layer is deliberately withheld from the document-level mirror, so the scoped `.liebe-portal-root` host remains the outstanding half of [Application mechanism](#application-mechanism).
...
All three are tracked by [0036](../../changes/0036-theming-contract-gaps.md), whose PR 1 has landed: the 44px touch floor the baseline lost to vendored CSS sharing its layer is restored, and vendored sheets now sit below the baseline — see [Application mechanism](#application-mechanism). One change, because all four are the same shape: the base layer knows something a theme or a baseline rule needs and exposes it through a name that cannot be reached.
...
- **What the contract covers.** The design-system anatomy classes (`liebe-card`, `liebe-icon`, `liebe-name`, `liebe-state`, `liebe-slider`, `liebe-pill`, `liebe-chip`, `liebe-value`, `liebe-spark`), the structural hooks `liebe-screen`, `liebe-section` and `liebe-section-title`, and the per-domain/state data attributes (`data-domain="light"`, `data-active`, `data-color="light|heat|…"`, `data-tier="glance|row|tall|full"`, `data-icon-tile`) MUST be treated as a public API: renames are breaking changes requiring a migration note here. Anything else a part renders — its inner elements, its internal custom properties — is not contract. Being named here is not the same as being in the markup: only the stamped set below carries a presence guarantee a theme may rely on today.
  ...
  - **Specified, not yet stamped — `data-icon-tile`.** The marker for the universal icon-only presentation ([entity-cards options — common](../entity-cards/options/common.md#icon-only-presentation-icononly)): presence-only, on the card shell, present exactly while the explicit `iconOnly` option renders the state-tinted icon-only tile ([design-system — card anatomy](../design-system/index.md#card-anatomy)). It is deliberately distinct from the internal centring attribute the derived `hideName`+`hideState` case stamps today, which is **not** contract and carries no tint — a theme restyling state-tinted tiles selects `data-icon-tile` and never reaches legacy both-hidden cards. Joins the stamped set when change [0033](../../changes/0033-icon-only-cards.md) ships.

- Themes MUST NOT rely on any selector outside this contract; internal class names MAY change freely.
- Domain color tokens are part of the contract as **triplets** (`--liebe-c-<name>`, `--liebe-c-<name>-tint`, `--liebe-c-<name>-text` — [design-system](../design-system/index.md#domain-color-discipline)): themes MAY remap them (LCARS does). Remapping only the base recolors the whole treatment through whichever companions the active theme left derived, and a theme MUST set `-text` explicitly rather than leave it derived, because the derived default is the raw base hue and carries no text contrast on any ground ([design-system](../design-system/index.md#domain-color-discipline)). **Default-theme caveat:** an explicitly set companion is an override like any other, and the Default theme pins each `-text` companion to Radix step 11 (raw step-9 hues lack text contrast). So under Default, user CSS remapping only a base recolors glyph and tint but leaves state text on the old hue — it SHOULD set `-text` too. Tints still follow automatically, since Default does not pin them.
...
- **A vendored stylesheet MUST land in a layer BELOW `liebe-base`, not in it.** The full order is `liebe-base.vendor` → `liebe-base` → `liebe-theme` → `liebe-user`, and every sheet the panel imports from a dependency (Radix Themes, react-grid-layout, react-resizable) goes in the first. Sharing a layer with Liebe's own baseline is not enough, because inside one layer the cascade falls back to specificity and a vendor sheet's class selectors outrank the element selectors a global baseline rule is written with. That is what silently unenforced the [44px touch floor](../design-system/index.md#card-anatomy): `app.css` declares it on bare `button` / `input` / `textarea`, Radix's `.rt-reset` declares `min-height: 0` on the same elements, so under an emulated coarse pointer a `size="3"` button measured 40px with a computed `min-height` of `0px` and a text field 38px at `auto`. Below the baseline, a Liebe rule wins by where it is rather than by out-specifying selectors that change on every dependency upgrade — which is also what makes Radix restyleable by the same cascade as everything else. **A sub-layer of `liebe-base` rather than a fourth top-level layer**, because a layer's position is fixed by the first `@layer` statement a root sees: a new top-level name would sort _after_ `liebe-user` in any root where a sheet still carrying the three-layer statement loaded first — the exact inversion of this rule, and invisible to a test that reads stylesheet text. A sub-layer is ordered by its parent and needs no statement, so the three layers this contract names stay the three a theme sees. **The floor MUST still lose to a token:** the anatomy's own sizes are class selectors in `liebe-base` itself, so the 34px `--liebe-chip-height` chip and the 38px pill stand, meeting the touch rule on the inline axis instead. Verified by measuring rendered controls under an emulated coarse pointer, because layer precedence is not observable from stylesheet text. **Done by [0036](../../changes/0036-theming-contract-gaps.md) PR 1.**
...
- **Portalled UI MUST stay inside the token scope.** Overlays portalled to `document.body` (dialogs, dropdowns — see [panel-lifecycle](../panel-lifecycle/)) land outside the shadow root and would render unthemed. Preferred: portal into a host element **inside** the shadow root. Where escaping the shadow root is genuinely required, the portal content MUST be wrapped in a `liebe-portal-root` container for which the engine mirrors all three layers at the document level (same CSS, selectors scoped to `.liebe-portal-root`, kept in sync on theme/appearance change). Either way, every Liebe overlay — including the detail dialog and its domain controls — renders with active tokens. **Not yet fully met — partly done by [0012](../../changes/0012-theming-engine.md):** the base and theme layers reach document-level portals (the bundled baseline is linked into the owning document as well as the shadow root, and the theme layer is mirrored there on every switch), so overlays render on the active palette. The **user layer is deliberately withheld** from that mirror: user CSS selectors are the author's own and nothing scopes them to Liebe, so a copy in the Home Assistant document would let a `body { display: none }` out of an imported configuration restyle the frontend around the panel. Containment wins over reach — the consequence, until the scoped `.liebe-portal-root` host above exists, is that a token override authored in custom CSS does not reach portalled overlays. Both halves are pinned by e2e assertions on a real open overlay.
...
│    …with liebe-base.vendor nested first inside liebe-base, so Liebe's own
│      baseline outranks the vendored sheets without out-specifying them
...
| 2026-07-30 | Vendored stylesheets moved into `liebe-base.vendor`, a sub-layer below the baseline: being layered was not enough, because inside one layer Radix's class selectors outranked the element selectors the 44px touch floor is written with                    | [0036](../../changes/0036-theming-contract-gaps.md)     |
