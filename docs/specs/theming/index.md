...
- The design-system anatomy classes (`liebe-card`, `liebe-icon`, `liebe-name`, `liebe-state`, `liebe-slider`, `liebe-pill`, `liebe-chip`, `liebe-value`, `liebe-spark`) plus structural hooks `liebe-screen`, `liebe-section`, `liebe-section-title`, and per-domain/state data attributes (`data-domain="light"`, `data-active`, `data-color="light|heat|…"`, `data-tier="glance|row|tall|full"`) MUST be present in rendered markup and MUST be treated as a public API: renames are breaking changes requiring a migration note here. **Stamped as of [0010](../../changes/0010-design-tokens-and-anatomy.md):** the nine anatomy classes and `data-domain` / `data-active` / `data-color`. Two entries in the list are not yet in the markup, for different reasons. **Deferred:** `data-tier` arrives with the [layout tiers](../../changes/0011-layout-tiers.md), since there is no tier to stamp until an effective span derives one. **Outstanding:** `liebe-screen`, `liebe-section` and `liebe-section-title` are stamped nowhere in the code. An earlier revision of this bullet listed them among the hooks 0010 shipped; that claim was incorrect — no change has ever stamped them. They stay in the contract because they remain the intended public API for framing screens and sections, and the natural consumer is a theme that draws section frames against them — [0013](../../changes/0013-built-in-themes.md) PR 2 (LCARS) specifies exactly such rules — but no change has been scoped to deliver them, so they are outstanding rather than scheduled. `data-active` is present **only** while a part is active, so themes select on the attribute (`[data-active]`) rather than on a value; `data-color` names the [domain colour triplet](../design-system/index.md#domain-color-discipline) the part's rendered state resolved to, and is what the base stylesheet maps onto the three `--liebe-c-*` tokens. Anything else a part renders — its inner elements, its internal custom properties — is not contract.
