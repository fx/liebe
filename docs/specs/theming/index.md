...
- Custom CSS does not reach portalled overlays — the user layer is deliberately withheld from the document-level mirror, so the scoped `.liebe-portal-root` host remains the outstanding half of [Application mechanism](#application-mechanism) ([#206](https://github.com/fx/liebe/issues/206)).
...
- A theme cannot recolour pill and chip labels: nothing in the contract — no selector, no token — reaches a label's colour, so the solid-fill-with-black-label treatment the LCARS section specifies is unreachable, and the shipped theme uses dark blocks of the domain hue instead ([#214](https://github.com/fx/liebe/issues/214)).
