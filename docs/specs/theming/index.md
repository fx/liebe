...
- Domain color tokens are part of the contract as **triplets** (`--liebe-c-<name>`, `--liebe-c-<name>-tint`, `--liebe-c-<name>-text` — [design-system](../design-system/index.md#domain-color-discipline)): themes MAY remap them (LCARS does). Remapping only the base recolors the whole treatment through whichever companions the active theme left derived, and a theme MUST set `-text` explicitly rather than leave it derived, because the derived default is the raw base hue and carries no text contrast on any ground ([design-system](../design-system/index.md#domain-color-discipline)). **Default-theme caveat:** an explicitly set companion is an override like any other, and the Default theme pins every `-text` companion (raw step-9 hues lack text contrast; which Radix step, per appearance, is the [design system](../design-system/index.md#domain-color-discipline)'s). So under Default, user CSS remapping only a base recolors glyph and tint but leaves state text on the old hue — it SHOULD set `-text` too, and for both appearances, since the pin it is replacing is per appearance. Tints still follow automatically, since Default does not pin them.
...
- **The baseline is three ordered tiers, because "the baseline" spans CSS with opposite needs**: `liebe-base.reset` (the universal reset) → `liebe-base.vendor` (every sheet the panel imports from a dependency — Radix Themes, react-grid-layout, react-resizable) → `liebe-base` itself (the token sheet, the anatomy, the component styles, the touch floor). All three still lose to `liebe-theme` and `liebe-user` as one block, so the three layers this contract names remain the three a theme reasons about. Each tier wins only over the one before it, and each of the two boundaries exists for a measured reason:
