...
- **No browser baseline is stated, and the two the build applies disagree.** [PRD](../../PRD.md) says "modern browsers only" with no version. That reads as "nothing is stated", and it is not: **Vite states one on the project's behalf.** `package.json` carries no `browserslist`, there is no `.browserslistrc`, and `vite.config.ts` sets no `build.target` — so Vite's default applies, `'baseline-widely-available'`. That name resolves differently between Vite releases, so the value below is read from the release **in the lockfile** rather than from documentation: on `vite@7.3.6` it is `chrome107, edge107, firefox104, safari16`, the `ESBUILD_BASELINE_WIDELY_AVAILABLE_TARGET` constant in `node_modules/vite/dist/node/chunks/logger.js`. Re-read it there after a Vite bump rather than trusting this line, and note that a reviewer quoting a different set is probably quoting a different release. Every production build down-levels JavaScript to that.
