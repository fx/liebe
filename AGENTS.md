...
2. **Probing a test (mutation testing)**
   ...
   **One more, deliberately left out of the seven because it is about the rig rather than the mutation: a reconstruction is cheap and sound for _confirming_ a mechanism you have already located, and unsound for _finding_ one.** Build a standalone harness that reproduces a composite from the stylesheets, and it will faithfully reproduce every rule you thought to include and silently omit whichever one the real DOM applies that you did not. The evidence is [0035](docs/changes/0035-light-appearance-contrast.md) PR 6, whose synthetic rig measured an LCARS live-hue glyph at **6.81–10.80:1** where the real card measures **2.33:1** — a factor of four, in the flattering direction, with nothing about the run looking wrong. The rig had the theme's stylesheet exactly right and the assembly wrong: `--liebe-part-color` resolved to the theme's own colour, because the rig never stamped the inline hue that the card puts on the part. So it measured a composite that does not occur, and reported it as the composite that does.

   Note that this is **not** a variant of the artifact-identity rule, and mistaking it for one is how it gets missed. The tells are opposite. Artifact identity is about the executing code being older or other than the source you changed, and it is caught by hashing what ran; here the artifact is current, the stylesheet is byte-correct, and hashing it confirms everything except the thing that is wrong. What a reconstruction cannot check is what it left **out** — and absence has no hash. The remedy is the one PR 7 used and it is not "make the harness more faithful", which is unfalsifiable: **measure through the product's own path** — a placed grid item, a real entity, the real frontend — so the assembly is not yours to get right. Reach for a harness once you know what you are looking at, to vary one input cheaply, and never to establish what the input is.

   **The same shape one level down, and the cheaper half to fix: a fixture is a claim about which cases matter, so a green run over a seed you chose is evidence about that seed and nothing wider.** Two instances in [0035](docs/changes/0035-light-appearance-contrast.md) PR 7, both harmless only because they went red. A colour sweep asserted its own requested `rgb_color` reached the card — and a Home Assistant light reports through `hs`, so a desaturated request comes back as pure white; the sweep was about to attribute a figure to a colour the dashboard never showed. And the same sweep, unprimed, satisfied its "wait until the value changes" poll against the state that was already there, labelling every row with the bulb **before** it: twelve real measurements, each attached to the wrong input, in a table that reads perfectly. **Read back what the system resolved rather than asserting what you asked for, and prime a change-detector before the first iteration** — an unprimed one is a tautology on its first pass and an off-by-one on every pass after. Prime it through a state the value cannot already be in (turning the light **off** first, here) rather than through another value, or the priming inherits the race it was added to remove.

   **The third instance is the one that earns the rule, because it was a live defect and the fixture was green.** The same PR shipped a CSS fallback picking a foreground against an **unbounded** input — any RGB a lamp can report — and qualified it against twenty hand-chosen hues, on which it agreed with `contrast-color()` twenty times out of twenty. `codex` review searched the space exhaustively and found `rgb(221, 5, 153)`, where a `clamp(0, delta * 100000, 255)` that reads as a binary switch is actually a **ramp**, returning a mid grey at **1.00:1** — worse than the value it replaced. Two things follow and the second is the general one. A saturating multiplier is not a branch; `round(up, x, 1)` is, because a ceiling maps every positive to 1 by construction rather than by steepness. And **where the input space is enumerable, enumerate it**: 2^24 colours is a couple of seconds of plain arithmetic, so the claim was split into an exhaustive test of the algorithm and a browser test that the engine computes that algorithm — which is stronger than any number of fixtures and cheaper than arguing about which ones matter. The rule above was written into this file **in the same PR**, hours before it failed; naming a failure mode confers no immunity to it, which is the lesson the changelog-table passage below records independently.
...
9. **Merging `main` into a long-lived branch: the changelog tables**
   ...
   **The rule itself was written wrong first, and the way it was caught is the point.** The correction above originally read "subtract each parent's own duplicate count", which is bad arithmetic: a baseline both parents share is **one** baseline, so subtracting it twice scores a correct merge at −1 and lets three headers where the parents had two score **0** — the exact case the check exists to catch. What makes it worth recording rather than quietly fixing is where the error was: the **script** that resolved the merge compared against the parents' maximum and was right, and only the **prose** was wrong. A rule and the tool that implements it drifted apart inside the paragraph warning about drift, and it took a reviewer reading the sentence against its own arithmetic to see it (PR [#330](https://github.com/fx/liebe/pull/330)). Read a rule here against what you actually ran, not against what you meant.

   **And the correction left one behind.** The paragraph above once ended by saying "the subtraction handles the header case" — still naming the mechanism by the superseded form, three lines below a bullet that now says in bold not to subtract, so a reader taking it at face value would implement the arithmetic this whole passage exists to correct. It survived the rewrite that forbade it, in the same paragraph, and was caught only on a second reading (PR [#333](https://github.com/fx/liebe/pull/333)). That is the strongest form of the point: naming a failure mode confers no immunity to it, and "re-read the whole enclosing unit, not the sentence you changed" is a rule that has now failed on the unit teaching it.

   Two forms of the fix work, and the first is better: **key rows within their own table**, so an identical row in a different table is a different key — which also handles two tables that legitimately share a _data_ row; or **exclude the header row of every table**, not only the separator, which does not. Neither is needed if the count is taken as "no duplicate the merge introduced" per the bullet above — **comparing against the parents' maximum** handles the header case, both instances above, and whatever the third turns out to be. The row-type exclusions are belt-and-braces for a check applied without it.
